      ...
      {data.map(stage => (
        <Collapse key={stage.stage} defaultActiveKey={['1']} ghost>
          <Panel
            header={
              <div
                className="flex justify-between items-center px-4 py-2 rounded-sm"
          ...
          >
            <div className="overflow-x-auto">
              {/* Table Header */}
              <div
                className="min-w-[600px] grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm font-semibold text-gray-500 px-4 py-2 border-b border-gray-300"
                style={{ backgroundColor: '#FDF2F9' }}
              >
                <div>Client</div>
                <div>Deal Name</div>
                <div className="hidden sm:block">Deal Budget</div>
              ...
              {stage.deals.map((deal, i) => (
                <div
                  key={i}
                  className="min-w-[600px] grid grid-cols-2 sm:grid-cols-4 gap-4 px-4 py-3 hover:bg-gray-50 border-b"
                >
                  {/* Client Info */}
                  <div className="flex items-center gap-2">
                    <Avatar style={{ backgroundColor: '#ccc' }}>{deal.clientInitials}</Avatar>
                  ...
                  {/* Deal Name */}
                  <div className="flex items-center">{deal.dealName}</div>

                  {/* Deal Budget */}
                  <div className="hidden sm:flex items-center">{deal.dealBudget}</div>
